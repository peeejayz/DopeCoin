...
// Reward: 100 coins per block, halved every 6 months
...
uint256 hashGenesisBlock("0x6377106f207fb1e86b653211baa9b9a63ca4eeda5d5a01923f8ea6ae043107fb");
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 2592) // no block reward within the first 3 days
        nSubsidy = 0;
    if(nHeight > 1576800) // no block reward after 5 years
        nSubsidy = 0;

    // Subsidy is cut in half every 157680 blocks, which will occur approximately every 6 months
    nSubsidy >>= (nHeight / 157680); // marketvaluedcoin: 157k blocks in ~6 months

    return nSubsidy + nFees;
}

int nTargetTimespan = 1 * 24 * 25; // 16 hours
static const int nTargetSpacing = 100; // 1.40 1min 40secounds
...
// Dopecoinfficulty adjustment protocol switch
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal dopecoin miner
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9356fb5ad51ff16c52dd1e3b6c0c6df0563390f902b01eb25569b7965db345d4");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Dopecoin Currency for a Digital Age";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391298698;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1842394;

        if (fTestNet)
        {
            block.nTime    = 1391295627;             //test net is supported
            block.nNonce   = 3024938;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xa0f89f016190243dd7efd70a7c3a0f560247ed4385346232fab4fb4587e1730c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // dopecoin: increase each by adding 2 to bitcoin's value.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // dopecoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
